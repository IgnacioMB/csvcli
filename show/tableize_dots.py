...
def tableize(df):
    ...
    total_filelines = 2 * df.shape[0] + 3

    content_lines = ['.'] * total_filelines + ['\n']

    content_lines[0] = hline
    content_lines[1] = build_data(df_columns, align_center)
    content_lines[2] = hline

    for index, row in df.iterrows():
        head = 3+2*index
        content_lines[head] = build_data(row.tolist(), align_right)
        content_lines[head+1] = hline
        os.system("clear")
        sys.stdout.write("\n".join(content_lines))


tabulated = tableize(df.head(100))
