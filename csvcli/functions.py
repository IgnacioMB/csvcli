...
def display_df(df, head=False, n=None):
    ...
    if not head:
        ...
        if n is not None:
            click.echo(pd_tabulate(df.head(n)))
        else:
            click.echo(pd_tabulate(df.head()))


def display_table(filepath, head=False, n=None, delimiter=','):
    ...
    display_df(df=df, head=head, n=n)


def display_columns(filepath, delimiter=','):

    df = pd.read_csv(filepath, delimiter=delimiter)

    cols_df = pd.DataFrame(df.dtypes).reset_index().rename(columns={'index': 'column_name', 0: 'dtype'})

    display_df(df=cols_df)


def display_summary_stats(filepath, delimiter=','):

    df = pd.read_csv(filepath, delimiter=delimiter)
    display_df(df=df.describe())


def display_null_columns(filepath, delimiter=','):

    df = pd.read_csv(filepath, delimiter=delimiter)

    null_df = df.isna().sum().reset_index().rename(columns={'index': 'column_name', 0: 'count_of_nulls'})

    display_df(df=null_df)
